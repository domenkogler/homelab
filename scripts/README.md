 ...
 **Vault item seeder wrapper** — pulls the write-scoped SA token transiently from the VPS (`/etc/op/provision-token`, HD-143) over SSH and runs `provision-secrets.py --create --yes`. `--dry-run` lists the catalog. Run from the WSL runner. | `docs/deployment-secrets.md` |
