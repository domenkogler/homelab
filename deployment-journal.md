...
### 2026-08-31 — Phase 1.5 · Ansible take-over dry-run fixes + collection-compat (HD-304 follow-up) `[AI]`

- **Status:** router + switch playbooks now pass `--check --diff` clean against the freshly-reset devices (`router.kogler.si` @ 10.10.99.1, `switch.kogler.si` @ 10.10.99.2, APs 10.10.99.90/91, laptop 10.10.99.98). **LIVE APPLY NOT YET RUN** — the operator asked for safe-mode/dry-run first; a live run needs a rollback backup per the safe-mode discussion + owner go-ahead.
- **Environment facts (blocked the run initially):**
  - `community.routeros` installed = **3.21.0**; `api_facts` no longer accepts `path:` (renamed to `gather_subset:`; `path` rejected with "Unsupported parameters").
  - Module needs **`librouteros`** in the interpreter Ansible uses. `inventory.ini` pins `ansible_python_interpreter=/usr/bin/python3` (system) which lacked it; the venv has it. Install: `~/ansible-venv/bin/pip install librouteros`.
  - `scripts/ansible-run.sh` hardcodes `REPO` to the **primary** checkout — cannot run a worktree branch. Use the direct `ansible-playbook` invocation from the worktree with `-e ansible_python_interpreter=~/ansible-venv/bin/python3`.
- **Fixes (all in worktree `homelab-wt-20260831-1054`, branch `session/ros-api-facts-fix`):**
  1. `roles/router/tasks/main.yml` + `roles/switch/tasks/main.yml`: `api_facts` `path: system identity` → `gather_subset: all`; identity assert reads `router/switch_identity_facts.ansible_facts.ansible_net_hostname` (3.x fact key; the old `data | map(attribute='name')` shape is gone).
  2. `roles/router/tasks/main.yml` DHCP-reservation loops: `selectattr('name','startswith','ap-')` → `match('^ap-')`; R-2 `selectattr('vlan','not in',...)` + `selectattr('name','not in',...)` → `rejectattr(...,'in',...)` (selectattr takes Jinja tests only).
  3. **HD-304 service-lockdown overreach fix:** the `available-from=` field is NOT supported by `community.routeros` 3.x for `ip service` (schema only has address/certificate/disabled/name/port/tls-version) → removed `available-from` from both roles' `Configure management services` tasks (kept disabled/port/certificate). The CIDR binding stays owned by the bootstrap `.rsc`; steady-state reachability = the INPUT-chain rules. Comment headers updated to say the role sets disabled-state, not the CIDR.
  4. `roles/switch/tasks/main.yml` `configured_port_names`: `map('extract','value','port')` → `map(attribute='value') | map(attribute='port')` (extract arg shape was wrong).
- **Verify (dry-run, laptop):** router `--check --diff` → `ok=32 changed=25 failed=0 skipped=1`; switch → `ok=22 changed=14 failed=0 skipped=1`. Identity asserts both PASS (correct device guard). `changed` counts are high because the devices are fresh-bootstrap (everything would be applied) — expected.
- **Model/schema lesson (folds into the parked post-cutover `validate-routeros7-syntax` linter idea):** the collection's `_api_data.py` `('ip','service')` field set is the authority on what `api_modify` can express; `available-from` (ROS7) is NOT in it. Role authors must check the module's field table before using a RouterOS-7-only parameter.
...
### 2026-08-31 — Phase 1.5 · wg-s2s bring-up attempt: router key corrected LIVE + VPS wireguard role live-applied + VPS peer-bind research (HD-306) `[AI]`

- **Plan ref:** `deployment-manual.md` §1.5.5 (wg-s2s handshake) + `todo.md` HD-285 + `todo.md` HD-306 (new research row). Continues the Phase 1.5 cutover after the owner completed the MikroTik password rotation (1P `mikrotik-admin_login` + `pppoe_login` rotated; devices updated).
- **Commands run (laptop → WSL runner, from session worktree `homelab-wt-20260831-1607`, venv interpreter):**
  ```bash
  bash scripts/ansible-run.sh playbooks/vps.yml --tags wireguard           # VPS wireguard role live-applied
  ansible-playbook -i inventory.ini playbooks/router.yml \
    -e ansible_python_interpreter=~/ansible-venv/bin/python3               # router role: corrected wg key + peer LIVE
  # router live converge: ok=32 changed=16 failed=0 ; VPS wireguard: ok=13 changed=5 failed=0
  ```
- **Router (RB4011, RouterOS 7.24.1) — read-only state verified via the mikrotik skill (routeros-api, rotated admin cred):** services lockdown exact (telnet/ftp/www/api-ssl/reverse-proxy disabled; api/ssh/winbox/www-ssl enabled, bound `available-from: 10.10.99.0/24`); ordered forward firewall (est/rel, CoAP 5683→HA, trusted-admin→IoT/Media, DNS resolver rules, VPS S2S scoped, Guest drop, Kids controls, default-deny last) + INPUT mgmt lockdown w/ SNMP + DNS tertiary — all as-authored. VLANs/interfaces/PPPoE present + running; Comtrend `/32` on ether1 (`192.168.1.2/32`). **Critical live finding:** the `wg-s2s` interface carried the **converge-.rsc auto-generated key** `/TITWs3wToml6tG4JVPiAwfXAj2RJDAzM+b9/xkRyi0=` (derive of `gwtI3HT5RI9JOHwtomctT5NfBzeneri7IIYzvTiAC7A=`) — NOT the shared `wg_password` pubkey `gwXAk+550CDfybOg64+v+QUFv1sW0wnGrIiI+EWFhUk=`. The `router` role's `Create WireGuard interface (wg-s2s)` task (sets `private-key` from 1P `wg_password`) corrected it live: post-run `/interface wireguard print` shows `public-key: gwXAk+...` + peer `allowed-address: 10.255.40.2/30,10.10.1.10/32,...,10.255.20.0/24` + `endpoint-address: 159.195.111.66` (VPS).
- **VPS (netcup, kernel 6.12.101, systemd 257) — wireguard role live-applied:** `wg-s2s.netdev` + `.network` rendered, systemd-networkd started/enabled, interface UP with `10.255.40.2/30` + correct public key `gwXAk+...` + listen 51820.
- **🔴 BLOCKED — VPS peer never attaches (HD-306 research):** even with the corrected `.netdev` `[WireGuardPeer]` (valid pubkey, `/32` allowed-ips, `Endpoint=s.kogler.si:51820` on its own line after the newline fix), `wg show all` shows **interface only, no peer**; `wg setconf` with a perfectly-formed `[Peer]` returns rc=0 but `wg showconf` still shows only the Interface; `lsmod` shows `wireguard 118784 0` (module refcount 0 → the netdev never bound to the wireguard driver). Tried: peer in `.netdev` (parsed, `netdev ready`, not applied), peer in `.network` `[Network]`-level `PeerPublicKey`/`PeerAllowedIPs`/`PeerEndpoint` (no-op; `.network` `[WireGuardPeer]` section itself → `Unknown section`, ignored on 257), `wg setconf` direct (silently drops peer). Root candidates recorded in the HD-306 row; next step is VPS-root research (manual `ip link add wg-s2s type wireguard`, networkd journal for the netlink create, or the documented wg-setconf-oneshot fallback).
- **IaC fixed + committed (worktree, this session):** ① Jinja `trim_blocks` newline-glue — Ansible's `template` ate the newline after `{% endfor %}` → `AllowedIPs=...10.255.20.0/24Endpoint=s.kogler.si:51820` on one line (invalid AllowedIPs) → fixed with explicit `{{ '\n' }}` (verified render `.../24\nEndpoint=...`); ② `.netdev` `mode: 0640` (was 0600 + ACL `mask::---` → networkd `Permission denied`); ③ `[WireGuardPeer]` moved from `.network` (257 ignores) to `.netdev` (correct documented location); ④ `wg_s2s_vps.endpoint: s.kogler.si` (DDNS token → current home WAN 193.77.156.222, idempotent + VPS-redo-safe); ⑤ doc drifts fixed: `deployment-manual.md` §1.5.5 port `13231`→`51820`, `IaC/router/README.md` rendered-files list (now 5 + converge pair). `validate-all.sh` green.
- **Secrets (1P items touched — values NEVER logged):** `mikrotik-admin_login` (read for API verify), `wg_password` (private key for both wireguard sides — referenced via role lookups; pubkeys derived + compared, values not logged).
- **Secret hygiene:** all rendered wireguard `.netdev`/conf files holding the private key removed from `/tmp` both local + VPS after the session; `IaC/router/rendered/` remains absent (post-rotation).
- **Deviations:** (1) did NOT complete the tunnel handshake (blocked by HD-306 peer-bind); (2) took a VPS-root research path for the peer rather than hacking around it in-session (correctness + timebox); (3) the router role run also re-applied the service-lockdown/INPUT parity (part of the 16 changed — expected drift-converge).
