...
#   * USABLE_FLOOR_GIB preflight + live guard, default 16 on the CORRECTED gauge
#     (usable = MemAvailable − CmaFree). The old run-scenario.sh floor of 8 GiB sits
#     INSIDE the danger zone, and it was measured on the WRONG gauge (see below).
...
PORT="${VLLM_PORT:-8000}"
# USABLE_FLOOR_GIB is the safety floor on the CORRECTED gauge (usable = MemAvailable −
# CmaFree). The box idles at usable ~18.5 GiB with the 16 GiB KV pool.
...
# §Second correction): the GPU carve is CMA and CMA counts as "available", so
# MemAvailable INFLATES under pressure — measured 160 sampler rows with CmaFree > 1 GiB
# (max 6.61 GiB), and at the two real kills MemAvailable read ~10.4 GiB while usable was
# 1.6. A guard on MemAvailable therefore trips EARLY (false GUARD FIRE at healthy
# 18.5 GiB usable) exactly when it reads low, and reads HEALTHY at 10.4 GiB when the box
# was already 1.6 GiB from death. Both errors are fixed by one gauge:
usable_gib() { awk '/^MemAvailable:/{a=$2}/^CmaFree:/{c=$2}END{printf "%.1f",(a-c)/1048576}' /proc/meminfo; }
# Kept for LOG TEXT only (it is what nvidia/top people grep for). Never gate on it.
...
guard_stop_engine() {
  ...
  for i in $(seq 1 24); do
    a=$(usable_gib)
    awk -v x="${a:-0}" -v f="$USABLE_FLOOR_GIB" 'BEGIN{exit !(x>=f)}' && break
    sleep 5
  done
  log "pool drained to usable ${a} GiB (>= floor ${USABLE_FLOOR_GIB}) — restarting $CONTAINER (guard aborts the run, not the service)"
...
guard_loop() {
  while :; do
    [ -f "$ABORT_MARK" ] && exit 0
    a=$(usable_gib); p=$(psi_full)
    awk -v x="${a:-99}" -v f="$USABLE_FLOOR_GIB" 'BEGIN{exit !(x<f)}' && guard_stop_engine "usable ${a} GiB < floor ${USABLE_FLOOR_GIB} GiB"
...
preflight() { # refuse to start/continue a step that is guaranteed to OOM the box
  local a p; a=$(usable_gib); p=$(psi_full)
  log "preflight: usable=${a}GiB MemAvailable=$(avail_gib)GiB PSI-full=${p}% engine-restarts=$(restarts) gpu-top=$(gpu_mib)MiB"
  awk -v x="${a:-0}" -v f="$USABLE_FLOOR_GIB" 'BEGIN{exit !(x<f)}' && die "only ${a} GiB usable (< ${USABLE_FLOOR_GIB} GiB floor) — would global-OOM the box"
...
serve() { # in out conc seed label
  ...
  log "STEP $label: in=$in out=$out conc=$conc seed=$seed usable=$(usable_gib)GiB"
  ...
  want=$(( in * conc / 2 ))
  log "STEP $label: rc=$rc ok=${ok:-?}/${conc} in_tok=${tok:-?} after: usable=$(usable_gib)GiB gpu-top=$(gpu_mib)MiB restarts=$r1 $([ "$r0" != "$r1" ] && echo '*** ENGINE DIED ***')"
...
log "=== stress-oom $TS | usable_floor=${USABLE_FLOOR_GIB}GiB gauge=MemAvailable-CmaFree psi_max=${PSI_FULL_MAX}% max_rung=$SPARK_STRESS_MAX_RUNG prefill_in=$BENCH_IN ==="
preflight
log "engine per-pid at REST: $(gpu_mib) MiB | usable $(usable_gib) GiB (MemAvailable $(avail_gib))   <-- the baseline every phase is judged against"
...
log "KV pool is FIXED at $(docker inspect "$CONTAINER" --format '{{join .Config.Cmd " " }}' 2>/dev/null | awk '{for(i=1;i<NF;i++) if($i=="--kv-cache-memory-bytes") printf "%.1f GiB", $(i+1)/1073741824}') — rungs below are sized by PREFILL, not by KV occupancy"
for rung in 1 2 3; do
  ...
  log "post-rung: usable=$(usable_gib)GiB gpu-top=$(gpu_mib)MiB — pausing ${RUNG_PAUSE}s to let the pool settle"
  sleep "$RUNG_PAUSE"
done

kill $GUARD 2>/dev/null || true
log "=== DONE. rest: usable=$(usable_gib)GiB gpu-top=$(gpu_mib)MiB restarts=$(restarts) ==="
