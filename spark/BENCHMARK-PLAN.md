...
| 10 | ~~KV offload connector (LMCache → `/mnt/spark_nvme/kv_cache`)~~ **REJECTED 2026-09-20 — not to be built; do not re-add** | — (was T4 infra) | — | (was: warm prefix cache across restarts; biggest TTFT lever on recurring long contexts) | **disqualifying, and it is silent:** LMCache serves this hybrid Mamba/GDN model only via `LMCacheMPConnector`, which **corrupts shared-prefix output on GB10 + Qwen3.x + FlashInfer with no error** (LMCache #4247, open; fix PR #4253 **unmerged**; #4674 still corrupts multi-session even with it), and its **persistence tier — the only part that paid here** — stores **1 of N kernel pages per chunk** (#4701): 95–99 % hit rates, wrong answers. Secondary: L1 pins RAM the pool does not have, and GB10 needs a source build → a new image digest. | **gate retired with the row:** the gate here was never TTFT — `TTFT ↓ ≥50%` **passes** the corruption. Re-decide only via the buried-fact-across-restart probe in [`docs/hardware-spark.md`](../docs/hardware-spark.md) §KV-cache persistence (LMCache) · decision log [`docs/services-rejected.md`](../docs/services-rejected.md) |
...
| #8 | vLLM | NVFP4 tables | | | | | | | |
| #9 | vLLM | GDN triton | | | | | | | |
| #10 | vLLM | **LMCache KV offload — REJECTED 2026-09-20, never run** (upstream correctness blocker on hybrid GDN, not a bench outcome — §6 #10) | — | — | — | — | — | — | — |
