...
| 3 | **HD-403** | Home Assistant gets an LLM to call: mint the vault-first `home-assistant` scoped key, then point HA's **stock `litellm` integration** (shipped 2026.8, already in the pinned `2026.8.1`) at `lan-litellm` and add its conversation agent to the Assist pipeline | ⚠ **Mechanism corrected 2026-09-26 — there is no `LITELLM_BASE_URL`:** URL + key go into HA's **config entry** (`.storage/core.config_entries`), config-flow only, no YAML import, so `templates/docker_services/home-assistant-primary/**` needs no change and no Pi render is gated on the item. ⚠ The key then sits in plaintext in `/config/.storage`, which the standby rsync copies to oldsrv. ⚠ The flow queries `/v1/models` **before** it creates the entry, so the mint must land first · [smart-home-voice.md](docs/smart-home-voice.md) |
