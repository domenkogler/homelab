...
|---|-----|-----------|--------------|
| 1 | **HD-377(a)** | **owner only:** open `https://stats.kogler.si` → **Homelab — LLM** and render it **again** (picker on All). The first render 2026-09-23 **failed** — "Throughput & Workload" and "Per-engine / TP-rank detail" were mostly No data — and the cause was template interpolation, not data: 15 of 53 panels were blank. Fixed, guarded and replayed query-by-query the same day; the two layouts the owner asked for ("Engine Internal & Cache", prefill/decode throughput) are now rows in the board. On sign-off the AI deletes `llm-inference-sglang-vllm.json` + `vllm-master-v2.json` + `vllm-dashboard.json` from `roles/monitoring/files/dashboards/` and re-converges `vps.yml --tags monitoring` | Park until the owner speaks (O4). Deleting the retired boards also retires the hard-coded `[5m]` prefix-cache gauge — **rebuilding that panel on a board scheduled for deletion is not the fix** |
