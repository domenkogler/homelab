...
| HD | P | ⏳ Next action | Why nothing blocks it |
|----|---|----------------|------------------------|
| **HD-413** | 2 | ~~Write the fail-loud assert~~ **Landed 2026-09-20** — `playbooks/tasks/self-converge-guard.yml` refuses `network`/`storage`/`wireguard`/`vps-hardening` when target == controller, in `pre_tasks` of all five plays; gate it twice (`check_self_converge_guard.py` + the executed 15-case `testdata/self-converge-guard/run.sh`), both in `validate-all.sh` | Safety net for the whole thread, and it already earns its keep: the executed matrix caught a tuple-vs-list bug (`ansible_run_tags == ['all']` is always false) that let an unfiltered self-converge of the netdev role through a version that passed every static reading. ⏳ owed: the runbook one-liner in `deployment-manual.md` (unowned file), and one real off-box converge from the VPS — which is HD-407's first act. ⛔ no `default()` / `failed_when` (HD-399 rule) |
