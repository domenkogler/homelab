...
| **Mint a current read+write 1Password service account** (`op-write_api`) | ~~HD-442~~ | **VOID — answered 2026-09-25: there is nothing to mint.** The SA is alive (the VPS reads real rows with the same item); oldsrv held a stale rendered file, and the refresh path is now in code. What remains is the oldsrv converge + re-probe + the cockpit item, all parked behind **HD-455** |
| ~~**Three SSH-grant verdicts**~~ | HD-443 | **CLOSED OUT 2026-09-25 (two rounds)** — Pi's admin surface = `ansible-admin` and the `admin` account is **not wanted** (key off it, then retire it, in that order); `ai_ssh` fleet-wide (no sudo, no op); `domen` on every node, sudo group with a password; 1Password item renamed to `domen_ssh` with the scripts moved in the same change. **One 30-second laptop step is yours:** rename the SSH-agent stub `~/.ssh/laptop-domen.pub` → `~/.ssh/domen_ssh.pub` and repoint `IdentityFile` |
...
| ~~**Decide whether oldsrv may push**~~ | HD-449 | **ANSWERED 2026-09-25 — you minted `github-homelab-deploy_ssh`, so push it is.** Nothing left to do in a browser; the AI installs it on the seat clone when oldsrv is back and proves it with a dry-run push (a deploy key is read-only unless write was ticked, so existence is not proof). Keep the read-only HTTPS token on the runner clone |
...
| **HD-442** | 1 | oldsrv `/etc/op/provision-token` is a **deleted** service account (`403 Service Account Deleted`) — re-mint, update `op-write_api`, re-run the pre-pass, then mint `cockpit-pi-web_api` with the value already on the box, and audit what silently never wrote **Gated on the box** — [docs/hardware-oldsrv.md](docs/hardware-oldsrv.md) §Reachability & wake.| **Yes** — minting the SA is yours |
| **HD-443** | 1 | the new grant gate is **red** until three placements are sanctioned in the doc or removed (Pi `admin` + NOPASSWD, `ai_ssh` on oldsrv, `domen_ssh` on spark); close the HA-guests coverage hole in the same pass | **Yes** — three verdicts |
...
| **HD-449** | 2 | answered 2026-09-25 (repo-scoped write key minted); the install + dry-run push wait on HD-455 — the oldsrv wording is now "push by seat, pull by runner". Kept aste key — and the docs still promised that ends when the cockpit lands, which it did; plus an unverified pre-restore runner key backup sits on the box | **Yes** — decide |
