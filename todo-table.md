...
| **AI / Office** ||||
| HD-391 | 2 | **Author + converge the three pinned-AI services decision #27 implies** — `docker_services/{whisper,reranker,embed}/` compose templates, **digest** pins in `group_vars/all/versions.yml`, sha256-verified GGUF/GGML model-fetch tasks, LiteLLM rerank row (`jina_ai/`) + embed re-pointed off `ollama/bge-m3`, sweep the dead pulls out of the ollama template + `vps.yml` | Research is closed: plan of record [`services-ai.md`](docs/services-ai.md) §3a, verbatim flags/values in [`services-ai-bench.md`](docs/services-ai-bench.md) §8 — this is now pure IaC + converge + live-verify |
| HD-392 | 2 | Diagnose **oldsrv SSH from the workstation** (Mgmt-direct and the Home-leg name both fail; a `vps` ProxyJump works) | Router/WireGuard/OS diagnostics are AI-runnable; **blocks the HD-391 converge** |
| HD-393 | 2 | Triage **641 × `amdgpu: init_user_pages: Failed to get user pages: -1`** (Sep 15→18, pre-dates the Vulkan work; from the existing KFD holders) | On-box `dmesg`/journal + per-holder KFD triage is AI |
