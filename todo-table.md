...
| **HD-147 `foto`** ✅ root-caused (2026-09-25) | The missing SSO button was `oauth.enabled` defaulting to **false**, not a missing client — a comment in the compose claimed "native OIDC enabled" and the flag was never set. Fixed in IaC; the login itself stays yours, and ⚠ **you must first say which account owns the family library**, because Immich links OIDC identities by email and Auto-Register opens a second empty library · [docs/deployment-oidc.md](docs/deployment-oidc.md) §Immich |

### A2. What still needs you — the honest residue

| What | HD | Note |
|---|---|---|
| **Browser logins** matrix / claw / cloud / foto / immich (+ register on Forgejo) | HD-147 + HD-141 tail | You know. The click-by-click with the five URLs comes with the next session's task list. **foto waits on one VPS converge, not on you.** Two embedded calls: **Forgejo primary + GitHub mirror** vs GitHub-only (Forgejo is **empty today**, so "primary" is a plan, not a fact), and **which Immich account owns the family library** before the first SSO login (a wrong email looks exactly like lost photos) |
