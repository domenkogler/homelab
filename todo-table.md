...
| HD-378 | 2 | Host Overview disk-I/O + temperature panels + enable Alloy `hwmon`/`thermal_zone` collectors | **Authored 2026-09-16.** Disk I/O needed no new telemetry (`diskstats` was already enabled; families verified in VM for `nvme0n1`) — the gap was that every board had capacity and none had work. ⏳ Two converges: `--tags monitoring` (panels) **+ the Alloy converge** (temps; `set_collectors` → `alloy.service` restart per host). No BIOS/kernel module needed for hwmon — the BIOS prerequisite belongs to the un-adopted `spark_hwmon` DKMS driver |
| HD-379 | 2 | Wire DCGM on spark (`spark-dcgm` loopback exporter + Alloy scrape/metric-whitelist) + tombstone the retired socat stack | **Authored 2026-09-16.** GPU telemetry had NO source on GB10 (no nvidia hwmon chip, nvidia-smi has no endpoint, VRAM impossible). Keep-list = the 6 signals measured real; profiling/`FB_*` refused by the exporter itself. ⏳ Converges: `spark.yml --tags monitoring,docker_services` + HD-378's Alloy converge on the other hosts + `vps.yml --tags monitoring` |
