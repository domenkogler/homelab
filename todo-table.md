...
| **HD-434** ⏳ | **KNX on the standby — narrowed to one hypothesis, no device visit needed to test it.** Ruled out by reading live state: discovery dependence, Pi-pinned `local_ip`, missing standby secrets (the config entry is unicast tunneling and `ha-config-sync` copies it). Left: both HAs claim `individual_address` 0.0.240, so the router may ignore a CONNECT for an IA it already holds from another source IP. Next: unicast CONNECT from oldsrv with a different IA — accepted means the fix is a per-standby IA in the runbook, refused means ETS. · [todo.md HD-434](todo.md) |

| **HD-437** ✅ | **Already implemented — my premise was wrong, and measuring the device caught it.** I decided the router advertised only the Pi and wrote it into three docs; live `/ip dhcp-server network` shows all six DHCP networks handing **three** resolvers (Home: Pi → VPS → oldsrv). No router change was made or needed. The real, narrower finding: IoT's `:53` is forced to the **Pi alone** and the Kids rules to the **VPS alone** — single targets by design, which is what their DHCP options cannot express. · [docs/network-dns.md](docs/network-dns.md) |
