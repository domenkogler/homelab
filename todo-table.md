...
| **HD-435** ⏳ | **The tailnet's HA answer must survive either home box** — ✅ the Pi is tailscale node 13 (`tag:home-edge`) and its `traefik-ha` answers `pi.ts.kogler.si` → 200 DIRECT (measured 2026-09-25) · ⛔ left: MagicDNS still answers `ha.ts.kogler.si` with oldsrv's DEAD node, so the dual-A publish of `ha.kogler.si` waits for HD-436's `tailnet: dual`; and the kill-the-node drill is unmeasured |
...
| **HD-403** | 2 | voice has an LLM to call | `oldsrv` back (HD-455 → HD-454) → flip `bootstrap_keys` → mint `home-assistant_api` → add HA's **stock `litellm`** integration + a conversation agent + the Assist wire → one Slovenian intent turn | ✅ **the owner call is CLOSED (research 2026-09-26):** HA **2026.8** ships a `litellm` conversation integration (ha-core #172960, merged 2026-07-17; probed at the tags — 404 @ 2026.7.0, 200 @ 2026.8.0) and **the pin `2026.8.1` already has it**: any proxy URL + a virtual key, so no vendored component, no HA bump, no relaxing #24. ⛔ Not a template change — HA reads no `LITELLM_BASE_URL`; URL + key live in `.storage/core.config_entries`, whose standby rsync replicates the key to oldsrv · [smart-home-voice.md](docs/smart-home-voice.md) · 📋 [`prompt-384.md`](prompt-384.md) |
...
- **The owner-side backlog is now five sentences long:** do the browser logins (HD-147), be present for the
  ...
  work while they wait), answer the one call still open from the 2026-09-26 session — **HD-465's node
  question** (does Cockpit hang off the Pi or off oldsrv). **HD-403's LLM-surface call is CLOSED by research:** HA
  2026.8 ships a stock `litellm` integration that takes any proxy URL + a virtual key and the pin `2026.8.1` already
  carries it, so voice needs no vendor, no bump and no relaxing decision #24 — it now waits only on `oldsrv`.
  **HD-464 is closed**: the classic Element
