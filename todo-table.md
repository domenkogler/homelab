...
| HD | P | ⏳ Next action | Why nothing blocks it |
|----|---|----------------|------------------------|
| **HD-413** | 2 | ~~Write the fail-loud assert~~ **Landed 2026-09-20** — `playbooks/tasks/self-converge-guard.yml` refuses `network`/`storage`/`wireguard`/`tailscale-node`/`vps-hardening` when target == controller, in `pre_tasks` of all five plays; gate it twice (`check_self_converge_guard.py` + the executed `testdata/self-converge-guard/run.sh` (it prints its own case count)), both in `validate-all.sh` | Safety net for the whole thread, and it already earns its keep: the executed matrix caught a tuple-vs-list bug (`ansible_run_tags == ['all']` is always false) that let an unfiltered self-converge of the netdev role through a version that passed every static reading. **The set grew once already, in flight:** HD-405's `tailscale-node` landed while this row was open and is the same class (it restarts `tailscaled`, and node-direct remote dev rides that interface) — so a new netdev/firewall/sshd/mount role is a lockout role whether or not anyone adds it here. ⏳ owed: the runbook one-liner in `deployment-manual.md` (unowned file), and one real off-box converge from the VPS — which is HD-407's first act. ⛔ no `default()` / `failed_when` (HD-399 rule) |
...
| **HD-407** | 1 | Owner seeds the `op` read-scope token + host keys on oldsrv → then bootstrap the runner and prove `--check` green **from oldsrv** per inventory group. **The prep landed 2026-09-20** (seeding recipe in §Runner placement, `--no-upgrade`/`--token-stdin`/`--no-sudoers` on `bootstrap-runner.sh`), and step 2 is **expected to REFUSE**: `/home/ansible-admin/.ssh/id_ed25519` on oldsrv holds a hand-made `oldsrv-rsync` key that is in no vault — identify what authorizes it before replacing it | An improvement, not just a move: `playbooks/dns.yml` is IP-filtered to the home WAN and its own text demands a *home* control plane (HD-397 measured the laptop failing that). The laptop runner stays until the first oldsrv-run log exists. Runner account = `ansible-admin` (token + fleet key + venv + clone); `domen` holds neither secret — it is HD-413's break-glass identity, and `ansible_user` is pinned in `host_vars` anyway |
