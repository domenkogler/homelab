...
**[prompt-407.md](prompt-407.md)** (runner + cockpit: HD-407/409/411 — the guardrail row it also carried is closed), **[prompt-412.md](prompt-412.md)**
(remote desktop: HD-412).

| HD | P | ⏳ Next action | Why nothing blocks it |
|----|---|----------------|------------------------|
| **HD-416** | 2 | Read-only `authorized_keys` audit across the managed hosts (fingerprints + comments only), diffed against the named-grant set in [deployment-secrets.md](docs/deployment-secrets.md) §Who is authorized where, failing loud on an unknown; then per hand-made key: give it a vault item or keep it hand-made **by decision** | Triggered by the `oldsrv-rsync` key found on HD-407's runner. Four live grants still have no vault item (`ha-sync@pi`, two `traefik-cert-sync`, plus whatever the next hand-run `ssh-keygen` leaves). ⛔ The audit must never edit an `authorized_keys` — the characteristic failure of an SSH-management tool is locking yourself out with your own fix |
| **HD-417** | 3 | Cell-count check for markdown tables in the docs validator (split on unescaped pipes, compare to the header row, fail with file + line) | Cheap gate for a failure that is invisible to every current validator: a dropped newline merged two registry rows on 2026-09-21 and a description with `/etc/x`-style alternatives added nine stray cells to a 3-column row. Needs an escape convention first — legacy rows quote pipes legitimately |
| **HD-405** | ✅ live 2026-09-20 | ~~Owner mints the headscale preauth key + tag and seeds the 1P item → then the node IaC, `tagOwners` in `policy.hujson`, and the §Tailnet-boundary rewrite~~ **done** — shipped as `roles/tailscale-node` (host tailscaled, node `oldsrv`, `tag:dev:443` only) + 1P `tailscale-oldsrv_api` + `tagOwners`/ACL + the §Tailnet-boundary rewrite + `ha.ts.kogler.si` on oldsrv's own edge; **⏳ owner's phone-on-cellular matrix** ([prompt-414.md](prompt-414.md)) (headscale stopped · IoT/guest unreachable · Companion on the `.ts` URL) + HD-410 capture in the same pass | The data path today is `phone → headscale → VPS edge → WG S2S → oldsrv`, so a VPS outage kills remote dev with the home link healthy. Home WAN is a **static public IPv4**, so P2P is available. ⛔ one host, no advertised routes, VLAN-99 seal untouched (HD-398 A stands) |
| **HD-414** | 1 | Configure the delegated /56 on the RB4011 through `roles/router` + the converge render — RA on the Home VLAN **only**, v6 filter = drop-all-from-WAN with one UDP 41641 accept to oldsrv — then probe inbound v6 **from the VPS** before calling the edge safe | The static /56 exists and the laptop is at home, so the router work is unblocked. Deliberately sequenced **before** HD-410 and HD-412: both are decisions about a path this changes. [prompt-414.md](prompt-414.md) |
| **HD-415** | 2 | Owner picks: leave the chain / reorder-trim it / split DNS per source | Nothing is broken — the away warning is the price of a WAN-out survival property, and the 2026-09-20 pass mis-called it a bug |
...
| **HD-407** | 1 | Owner seeds the `op` read-scope token + host keys on oldsrv → then bootstrap the runner and prove `--check` green **from oldsrv** per inventory group. **The prep landed 2026-09-20** (seeding recipe in §Runner placement, `--no-upgrade`/`--token-stdin`/`--no-sudoers` on `bootstrap-runner.sh`) and **the blocker in front of it is cleared**: the hand-made `oldsrv-rsync` key in `/home/ansible-admin/.ssh/` was retired 2026-09-21 — its public half authorized `ansible-admin` on nas, which is NOPASSWD root there, and nas's sshd recorded no use after 2026-09-08 11:56 — so `restore-runner-key.sh` now finds an empty path (delete both halves after one green backup night) | An improvement, not just a move: `playbooks/dns.yml` is IP-filtered to the home WAN and its own text demands a *home* control plane (HD-397 measured the laptop failing that). The laptop runner stays until the first oldsrv-run log exists. Runner account = `ansible-admin` (token + fleet key + venv + clone); `domen` holds neither secret — it is the self-converge guardrail's break-glass identity, and `ansible_user` is pinned in `host_vars` anyway |
...
| HD-403 | 2 | Give HA the gateway path: mint a `home-assistant` scoped key vault-first, render `LITELLM_BASE_URL` + key into the compose env, point the Assist pipeline at it | Voice's LLM leg currently has **nothing to call** — HA ships only `TZ` in its env, has no consumer in either LiteLLM and no `llm:` config in its role; rides with HD-384 · [smart-home-voice.md](docs/smart-home-voice.md) |
| HD-404 | 3 | Text-only PR: fix each stale string enumerated in the row (Victoria headers in `home_servers.yml`/`playbooks/storage.yml`, the Pi "primary DNS" comment, the `llm-backend` purpose string that feeds the generated address doc, `amd_rocm`'s `OLLAMA_KEEP_ALIVE`, `first-boot-config.sh`'s `raspi.debian.net` wording) | The docs stopped repeating these, so the comments are now the last place an agent reads the wrong truth · [deployment-ansible.md](docs/deployment-ansible.md) |
