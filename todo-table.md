...
| **Bring oldsrv back on the network** | HD-361 · HD-442 · HD-444 · HD-445 | **four lanes wait on this one box** and no AI step substitutes for it: WoL from the nas, or the Comet KVM if it is hung rather than off (so no hand on a button is needed). Wake path, MAC and the evidence trail: [docs/hardware-oldsrv.md](docs/hardware-oldsrv.md) §Reachability & wake |
