...
| **HD-406** ✅ | Build **now: HD-414 is measured and punching is dead on this SIM, so phone-initiated WireGuard is the shape that survives**; the phone peer is **scoped** to one admin device, with **full-tunnel as an optional second allowed-IPs profile** — that profile is what answers Slovenian egress and what makes the Pi's exit node redundant. AI mints the keypair | `todo.md` row |
