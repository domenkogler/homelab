...
| HD-335 / HD-337 | 2 | spark (ThinkStation PGX / GB10) bring-up | **PROVISIONED + LIVE 2026-09-14/15** (DGX OS, bootstrap, XFS carve, engine B1 up + serving `spark/qwen3.8-flash-next`; spark name edge + LiteLLM names live via HD-370, commits `0400f78`+). Remaining lane = **bench S1–S5** (HD-367) + Triton/Mem0/OpenHands; the LiteLLM completion round-trip is now LOCAL-spark-capable once the owner adds the Admin-UI model entry |
