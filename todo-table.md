...
| ~~**Three SSH-grant verdicts**~~ | HD-443 | **ANSWERED 2026-09-25** — Pi's admin surface = `ansible-admin`; `ai_ssh` fleet-wide (no sudo, no op); `domen` as a real account on every node incl. spark, with the item renamed to `domen_ssh`. One click still yours: the 1Password rename. One sub-question left open: does the key come **off** the Pi's second `admin` account (recommended — IaC needs nothing there) or is it sanctioned as-is |
