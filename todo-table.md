...
| **HD-391 tail** | 1 | **#27 is RATIFIED (2026-09-18)** — the owner call that is left inside it: the reranker has **no live consumer** (`rag-mcp` is `enabled: false` on the VPS). Ship the rerank leg with the tier anyway, or park it until the RAG reader is enabled? | The HD-391 authoring proceeds either way; this only decides whether the third service ships enabled |
