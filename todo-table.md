...
| **HD-415** ✅ | Neither "leave it" nor "trim it": make the resolver **reachable from wherever the device is**. The chain becomes **oldsrv's tailnet node address** (direct-over-LAN at home with the WAN pulled; reachable away while the home link is up) **+ the VPS public instance as fallback**, and the two LAN-address entries go. Case (d) — away while the **home** WAN is down — is accepted as unresolvable: your words, *"it is ok that I will not get dns for whole domain, because it is not reachable from the internet."* Folded into the HD-414 lane · **✅ shipped 2026-09-22** — the chain is live and answers over the tailnet; ⏳ the three-case drill remains | [network-dns.md](docs/network-dns.md) §The resolution requirement + [network-rejected.md](docs/network-rejected.md) 2026-09-21 |
| **HD-415 · second call** ✅ | **Include `udp 53`** — the 2026-09-20 "443 ONLY" freeze is lifted **for DNS only**, written `udp:`-scoped to the resolver node rather than to `tag:dev`; no LAN bridge, no advertised routes, no exit node, no Tailscale SSH. The headscale stop/start is **authorized with a hard safety condition: if the driving session drops, headscale must come back on by itself** (prove the auto-re-enable before the first stop) — **both delivered 2026-09-22**: `scripts/restart-watchdog.sh` + `scripts/guarded-converge.sh`, proved live (one stop, self-recovered in 19 s) before the single authorized restart | [`todo.md HD-415`](todo.md) |
| **HD-432** ⏳ | **Undecided, and it is the owner's call:** route the internal zone to a tailnet resolver via `dns.nameservers.split`, or leave it advisory. Measured: with `override_local_dns: false` clients keep their local resolver for qualified names, so the shipped chain is never asked and cellular gets carrier NXDOMAIN for names the node answers over the tailnet. Splitting `kogler.si` fixes that and also ends the VPS-instance NXDOMAIN race, at the price of **no fallback** while oldsrv's node is down. Needs a second guarded restart. |
| **HD-433** ⏳ | **Undecided shape for one HA URL everywhere.** At home the answer is `ha.kogler.si` (VIP, Pi, survives oldsrv and a WAN pull). Away, the answer must be reachable from away: the Pi's own tailnet node survives oldsrv; the VPS edge survives both proxies but dies when the home WAN dies; the only answer that *floats* is a tailnet-routable VIP, which costs the no-routes-to-user-nodes invariant. **HD-405 T1 is now honestly verified** (2026-09-22, owner's phone, Wi-Fi × Tailscale matrix: `ha.ts.kogler.si` serves HA on cellular). What remains is which single URL the HA Companion app carries. |
