...
| ~~Re-measure the phone once~~ **RAN 2026-09-22: still relayed** | HD-410 · HD-414 tail | Phone→oldsrv **FRA 70–90 ms**, phone→VPS **NUE 50–70 ms**, and from home the reciprocal ping is **`direct connection not established`** (73–244 ms). IPv6 did not buy a direct session; **the no-DERP decision stands**. ⏳ One 60-second follow-up needs the phone online: scratch v6 accept above the WAN drop → re-ping → delete; it is the only thing that separates "carrier blocks the phone" from "our filter blocks the phone's probe". ⚠ Separate find, **no row yet**: the VPS's tailscaled listens on ephemeral ports in a container netns with nothing published, so that node can never be direct — pin + publish it for the latency win. |
...
| **HD-412** | 2 | family machines can be helped remotely | 🟢 **shipped 2026-09-21** — server LIVE on the VPS; two live sessions remain (one relayed, one Path-A with the relay uninvolved) | model **decided**: family machines join the tailnet where they can, VPS rendezvous + **bandwidth-capped** relay for the rest. ⛔ never on oldsrv: a rescue tool behind the thing it rescues is not a rescue · 📋 prompt-412.md — brief deleted |
...
| **HD-415** | 3 | one tailnet resolver that works **wherever the device is** | **unblocked 2026-09-22:** add `udp 53` to the headscale ACL (`udp:`-scoped to the resolver node, not to `tag:dev`), bind Technitium to oldsrv's **tailnet node address**, make it the headscale nameserver, keep the VPS instance as the fallback, drop the two LAN-address entries | **decided 2026-09-21, authorized 2026-09-22** (ACL `udp 53` + the headscale restart) — ⛔ build the **session-drop auto-re-enable first and prove it**, then one restart. ⚠ acceptance = the three-case drill (LAN / cellular / **home with the WAN pulled**) · [network-dns.md](docs/network-dns.md) §The resolution requirement · 📋 [`prompt-414.md`](prompt-414.md) |
...
- **Then the sequenced chain:** HD-414 (scoped IPv6) was the hinge — it shipped and measured, closing itself and HD-410, which unblocks
  HD-415 resolver redesign: HD-406 now runs and is the surviving transport shape, while HD-415 waits on an owner ACL call rather than on a measurement (HD-412 shipped 2026-09-21 without waiting). HD-407 is unblocked and mechanical now, and HD-409 rides it.
