...
- **HD-342 — Victoria stack deploy (VPS).** IaC done (VM/VL replace Prometheus/Loki, topology B), **nothing live yet**. ⏳ Seed `victoria-metrics_api`/`victoria-logs_api` → converge VPS (owner) → verify datasources/dashboards/alerts → kopia client wiring. **Coordination:** owner runs the VPS converge; other sessions must NOT converge VPS. [observability.md](docs/observability.md) · [todo.md HD-342](todo.md)
