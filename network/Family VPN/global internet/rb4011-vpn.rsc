# 2026-06-30 16:13:37 by RouterOS 7.23.1
...
/interface wireguard add comment=back-to-home-vpn listen-port=38261 mtu=1420 name=back-to-home-vpn
/interface wireguard add listen-port=13231 mtu=1420 name=wg-home
...
/ipv6 settings set max-neighbor-entries=8192 soft-max-neighbor-entries=8191
...
/interface wireguard peers add allowed-address=10.10.99.2/32 comment="Domen P14s" interface=wg-home name=peer1 public-key="RHck0UG8qgpJS5heATXymKX9RYLe+qJSS14ysXcE2jU="
...
/ip address add address=10.10.99.1/24 interface=wg-home network=10.10.99.0
...
/ip cloud back-to-home-user add allow-lan=yes comment="router | RB4011iGS+" name="Tablica Valentina" public-key="0FDRm0pmL66FpJTOiwurN0bqN/oUDw2xUksAJOagglk="
/ip cloud back-to-home-user add allow-lan=yes comment="router | RB4011iGS+" name="Naprava A54 uporabnika Domen" public-key="QauRBEFXV4u3yjOuStoAdXgzGa1hBOnLhQeWcytd1mo="
/ip dhcp-client add interface=ether1 name=ether1
/ip dhcp-server add add-arp=yes address-pool=dhcp_pool-10.10.10 interface=*B0 name=dchp-10.10.10
...
/ip dhcp-server lease add address=10.10.1.125 client-id=ff:5d:1:67:1e:0:1:0:1:31:d0:22:8e:0:15:5d:1:67:1e mac-address=00:15:5D:01:67:1E server=dhcp-10.10.1
...
/ip firewall filter add action=accept chain=forward comment="VPN -> LAN" in-interface=wg-home out-interface-list=LAN
/ip firewall filter add action=accept chain=forward comment="LAN -> VPN" in-interface-list=LAN out-interface=wg-home
...
/ipv6 address add from-pool=pool_v6 interface=bridge
/ipv6 address add address=0:0:0:1:: from-pool=pool_v6 interface=ether6
...
/system routerboard settings set auto-upgrade=yes boot-delay=9s
