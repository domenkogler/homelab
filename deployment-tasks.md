...
- [ ] **HD-47** — Matrix public records + `_matrix` well-known/SRV delegation. Records + the `matrix.` well-knowns are live and publicly resolvable; the blocker found 2026-09-25 is that the **apex** `/.well-known/matrix/*` answers 302 into Forward-Auth, which is what a remote homeserver actually asks. · [services-traefik.md](docs/services-traefik.md)
- [ ] **HD-122** — Matrix federation hardening: ✅ profile endpoints verified requiring auth (401 `M_MISSING_TOKEN`, 2026-09-25); inbound federation still unproven (rides HD-47). · [services-matrix.md](docs/services-matrix.md)
...
- [ ] **HD-141 / HD-147** — the Authentik OIDC epic (Blueprint + secret-egress glue) and its live deploy-verify tail. Proven by a real login: `chat` (matrix), `file`, `foto`, `git` (all 2026-09-25; the `foto` fix is NOT env — Immich v3 has no OAuth env vars, see deployment-oidc.md §Immich). `vpn` was verified in the HD-219 era and has not been re-checked. ❌ **`ai` is broken** — callback path moved in Open WebUI 0.11 (**HD-458**). Still open: `claw`/`cloud` desktop+mobile, and proving Forgejo gets any content at all. · [deployment-oidc.md](docs/deployment-oidc.md)
