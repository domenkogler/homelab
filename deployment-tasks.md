...
- **HD-374 (spark vLLM unified-memory budget governance — 3rd global-OOM in 24 h)** — ✅ **IaC DONE + MERGED 2026-09-16:** explicit `--kv-cache-memory-bytes 8800000000` in `group_vars/spark.yml` + compose arg (the ONLY governor — this build ignores `gpu_memory_utilization` when the bytes flag is set, so util is REMOVED); host reserve ~21.9→~32.5 GiB; `max_model_len` unchanged — boot-gate only. ⏳ **Deploy-gate:** converge spark → verify engine boots (KV ≈ 275k tok, no `ValueError`), `MemAvailable` ≥ ~30 GiB idle, `/health` 200 → monitor. Authoring tail: spark host-memory alert rules (`spark-host-mem-oom-warning/critical`) in `roles/monitoring/vars/main.yml`. Knowledge: `docs/hardware-spark.md` §Unified-memory budget · incidents: `docs/spark-incidents.md` · todo HD-374.
