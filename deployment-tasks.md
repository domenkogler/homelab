...
- **HD-370 (spark name edge + LiteLLM names)** — ✅ **ALL LEGS LIVE 2026-09-15** (VPS/oldsrv/Pi converged failed=0; VPS pre-pass `--check` artifact fixed; DNS split-horizon verified — `llm`/`db-spark`/`spark` resolve to spark on home instances only, `litellm`→public on the primary; TLS-in-TLS hop verified with the bearer key → `spark/qwen3.8-flash-next`; wireguard oneshot now installs routes). ⏳ **Owner gate:** Admin-UI — create `spark/qwen3.8-flash-next` in BOTH LiteLLMs (VPS `https://llm.kogler.si` / LAN `http://llm.kogler.si`) + `spark-llm_api` (VPS `LiteLLM_ModelTable` verified EMPTY) + confirm LAN glue minted dsh/pi-harness keys. Spec: `docs/hardware-spark.md` §Name edge · `docs/services-ai.md` §2 · `todo.md` HD-370.
- **HD-371 (tailnet MagicDNS-locally)** — ⏳ **IaC done 2026-09-15, ⏳ deploy-gated:** VPS `docker_services` converge (scope=headscale) → restart headscale → verify from the laptop-on-hotspot: `nslookup llm.kogler.si` → `100.64.0.1`, `nslookup stats.kogler.si` → `100.64.0.1`, `nslookup litellm.kogler.si` → `100.64.0.1`; phone + laptop both resolve + reach (`llm` 401 / `stats` 302 / `litellm` 200). Spec: [network-vpn.md](docs/network-vpn.md) · [network-dns.md](docs/network-dns.md) · `todo.md` HD-371.
- **HD-372 (litellm.kogler.si 502)** — ⏳ **IaC done 2026-09-15, ⏳ deploy-gated:** VPS `docker_services` converge (scope=litellm + traefik-tailnet) → verify from a tailnet device: `https://litellm.kogler.si` → 200 (LiteLLM login, no 502). Spec: [services-ai.md](docs/services-ai.md) · [network-vpn.md](docs/network-vpn.md) · `todo.md` HD-372.
...
3. **Mandatory hardening checklist (HD-154, enforced by the `vps-hardening` role — NOT optional prose):**
   ...
   - **Firewall (nftables):** default-deny inbound; allow only `:22` (SSH/Ansible — added 2026-08-22 after a live lockout) + `:443` (Traefik) + `:51820` (WG S2S) + loopback + established/related; ICMP echo limited. Role-deployed `/etc/nftables.conf`.
