...
- [ ] **HD-47** — Matrix federation: prove **inbound delivery** with one external-room join (owner step). The delegation itself is live — both apex `/.well-known/matrix/*` read 200 off-network while the launchpad stays behind Forward-Auth. · [services-matrix.md](docs/services-matrix.md)
