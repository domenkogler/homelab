...
- **HD-240** — grafana SSO round-trip: traefik edge IP stays pinned `.2` (= `GF_AUTH_PROXY_WHITELIST` match); **[MANUAL]** map the owner's Authentik email into Grafana via the break-glass admin API (`AUTO_SIGN_UP=false`, HD-190); then a logged-in browser click sso-dashboard → stats.kogler.si must land IN Grafana, never on the logo-only `/login`. · [observability.md](docs/observability.md)
