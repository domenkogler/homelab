...
- **KNX config direction (decided 2026-08-21): ETS project file (`project_file:`), not hand-maintained YAML GA maps.** The ETS export **`assets/references/knx/StanovanjeKogler_v1_0.knxproj`** is the SSOT for group addresses — HA's `knx` integration loads it via `project_file:` so addresses resolve names/dtypes straight from ETS (entity configs may reference GAs by project name). The HD-04 redo deploys the file with the `home_assistant` role and renders `knx:` against it. The harvested YAML GA maps (`assets/references/old-ha/knx-*.yaml`) are the **legacy** live-config reference only.
