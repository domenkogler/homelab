...
- Certificates: wildcard `*.kogler.si` via ACME **DNS-01** (Cloudflare API token in 1Password `Homelab-ansible`). **Issuer = the VPS Traefik** (HD-178 — the single issuer; oldsrv's internal edge and the Pi `traefik-ha` consume the synced pair; template enforcement tracked HD-181 ⏳).
