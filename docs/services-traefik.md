...
- **Identity gap (HD-361):** Cockpit is PAM-only — a working login needs a Linux account
  ...
  so the cockpit-* UIs have **no valid credential**. **Decided 2026-09-21 (HD-361):** one dedicated
  ...
  so these converges run from the laptop, never oldsrv-converging-itself. Until it lands the cockpit routes are
  reachable but unauthenticable by design.
