...
- **Live status 2026-08-24 (HD-166 tail, NOT closed — fix in-flight):** stack deployed + healthy (onlyoffice-*docs* up, JWT+WOPI+collab env set, `office.kogler.si` 200), but owner round-trip shows `.txt` preview/edit/save works while **`.docx` fails — “No preview available … download instead”** (ONLYOFFICE editor never opens office docs).
  - **Root cause (found live 2026-08-24):** the OpenCloud `collaboration` service **never starts**. The compose sets BOTH `OC_ADD_RUN_SERVICES: collaboration` AND `OC_EXCLUDE_RUN_SERVICES: collaboration,idp`. In OpenCloud 7.4 the running set is `(defaults − EXCLUDE + ADD)`; a service named in EXCLUDE is NOT started even if also listed in ADD. `opencloud list` on the live box confirms `collaboration` is absent (while `nats` embedded, app-registry, gateway, frontend all run). Because `collaboration` never registers its WOPI app with app-registry, the web UI has no editor for office MIME → `.docx` falls to “download instead”.
  ...
  - **Secondary (unresolved):** the rendered `/etc/opencloud/opencloud.yaml` shows `collaboration.app.insecure: true` despite env `COLLABORATION_APP_INSECURE: "false"` — the env→schema mapping for this flag does not appear to take effect in the 7.4 rolling build. Flagged as a follow-up, not yet fixed.
  - **Fix (in-flight, Lane A IaC):** remove `collaboration` from `OC_EXCLUDE_RUN_SERVICES` (keep `idp` excluded), so ADD actually starts it; then **converge + live re-test the `.docx` round-trip (PENDING)**. See deployment-journal.md HD-166 pt.2.
