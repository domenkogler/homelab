...
- **Live status 2026-08-24 (HD-166 tail — THREE layers diagnosed; pt.1–3 fixed+deployed, pt.4 diagnosed + one more fix pending):** stack deployed + healthy, `.docx` no longer “download instead” — the editor NOW opens via ONLYOFFICE/WOPI (discovery 200) but shows **“Invalid access token” (401)**.
  - **Root cause #1 (found live, DEPLOYED):** the OpenCloud `collaboration` service **never started** — compose set BOTH `OC_ADD_RUN_SERVICES: collaboration` AND `OC_EXCLUDE_RUN_SERVICES: collaboration,idp`; running set = defaults − EXCLUDE + ADD, so EXCLUDE netted it out. `OC_EXCLUDE_RUN_SERVICES` → `"idp"` only (deployed).
  - **Root cause #2 (found live, DEPLOYED):** ONLYOFFICE DS had **WOPI disabled** — DS image defaults `WOPI_ENABLED=false`, so `local.json` had `wopi.enable:false` and `/hosting/discovery` 404'd. Added `WOPI_ENABLED: "true"` (deployed; discovery now 200, keypair auto-generated).
  - **Root cause #3 (DIAGNOSED, PARTIALLY fixed — IN-FLIGHT):** `COLLABORATION_WOPI_SECRET` was unset → added = shared value (deployed, passes the WOPI leg). Remaining: **`OC_JWT_SECRET` env is ABSENT** → reva `DismantleToken` fails (`token signature is invalid`, wopicontext.go L125) because auth mints the REVA token with yaml `token_manager.jwt_secret` (`eefff1c1…`) while collaboration verifies with `COLLABORATION_JWT_SECRET` env (`169e85ad…`). **Fix pending: set `OC_JWT_SECRET` = same shared value on the opencloud compose, converge, then user RE-LOGIN + retry.** See deployment-journal.md HD-166 pt.2–pt.4.
