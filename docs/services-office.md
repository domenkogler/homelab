...
- **Live status 2026-08-24 (HD-166 tail — TWO root causes found & fixed live; owner browser re-test PENDING):** stack deployed + healthy (onlyoffice-*docs* up, JWT+WOPI+collab env set, `office.kogler.si` 200), but owner round-trip showed `.txt` preview/edit/save works while **`.docx` failed — “No preview available … download instead”** (ONLYOFFICE editor never opens office docs).
  - **Root cause #1 (found live):** the OpenCloud `collaboration` service **never started**. The compose set BOTH `OC_ADD_RUN_SERVICES: collaboration` AND `OC_EXCLUDE_RUN_SERVICES: collaboration,idp`. In OpenCloud 7.4 the running set is `(defaults − EXCLUDE + ADD)`; a service named in EXCLUDE is NOT started even if also listed in ADD. `opencloud list` confirmed `collaboration` absent (while `nats` embedded, app-registry, gateway, frontend all run). **Fixed (Lane A, deployed):** `OC_EXCLUDE_RUN_SERVICES` → `"idp"` only.
  - **Root cause #2 (found live after #1):** ONLYOFFICE DS had **WOPI disabled** — the DS image defaults `WOPI_ENABLED=false` (`run-document-server.sh`), so `/etc/onlyoffice/documentserver/local.json` had `wopi.enable:false` and DS never served `/hosting/discovery` → OpenCloud's collaboration polled it, got 404, and couldn't register the editor. **Fixed (Lane A2, deployed):** `WOPI_ENABLED: "true"` in the onlyoffice-docs compose; DS now serves `/hosting/discovery` **200** with the full Word/Excel/PowerPoint discovery XML and auto-generates its WOPI keypair.
  - **The earlier “no NATS broker / app-provider not authoring” theory was DISPROVEN:** the embedded NATS service **is** running (`opencloud list` shows `nats`), so no separate NATS container is needed, and `OC_ADD` does NOT win over `OC_EXCLUDE`.
  - **Secondary (unresolved):** rendered `/etc/opencloud/opencloud.yaml` shows `collaboration.app.insecure: true` despite env `COLLABORATION_APP_INSECURE: "false"` — the env→schema mapping for this flag does not appear to take effect in the 7.4 rolling build. Flagged as a follow-up, not yet fixed.
  - **Status after both fixes (2026-08-24, converge 9P ✓):** `collaboration` running, `/hosting/discovery` 200, no new discovery errors in opencloud logs. **Owner browser `.docx` round-trip re-test is the closing gate — PENDING.** See deployment-journal.md HD-166 pt.2/pt.3.
