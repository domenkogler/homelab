...
> **Current state (2026-09-03):** ✅ **Debian 13 (Trixie, XFCE) INSTALLED 2026-08-23** (Phase 1a reinstall done — interactively, not via preseed; execution record: [deployment-tasks.md §Phase 1a](../deployment-tasks.md)). ✅ **2026-09-03: oldsrv set to its SSOT static IPs + tagged Mgmt leg** (first Ansible-contact pre-step — see [deployment-tasks.md §Phase 3](../deployment-tasks.md)): NM profile `Wired connection 1` → `manual` (SSOT `oldsrv` Home row + [`network-addresses-generated.md`](network-addresses-generated.md) gateway, DNS 1.1.1.1/1.0.0.1, `dhcp-client-id=mac`) + new NM VLAN profile `VLAN 99 Mgmt` (`enp0s31f6.99`, SSOT `oldsrv` Mgmt row, `never-default`). ⚠ **Blocked before full provision:** the tagged-99 **mgmt plane is DOWN cluster-wide** (router SSH down on both legs; SSOT `router`/`switch`/`ups` rows ARP-FAILED from the Pi's tagged leg — see [network-ops.md](network-ops.md) incident 2026-09-03). ⚠ **switch_port_map wiring gap:** `switch.yml` has **no `oldsrv` entry** — host_vars says `eno1 → CRS328 ether11`, but the map assigns `ether11` to `ap-dnevna` (needs resolution once the mgmt plane is up). 6 oldsrv 1P items missing (`sonarr_api`/`radarr_api`/`pihole_password` auto + `privado-vpn_api`/`signal_api`/`ha_api` human) — see [deployment-secrets.md](deployment-secrets.md). Boot mode **BIOS/CSM** (msdos table on the 960 EVO; UEFI-force question answered No). Ansible hold rule active — first playbook contact only after the Phase 1.5 cutover. Storage pools (ZFS `nvme`, NFS mounts) are NOT live yet.
