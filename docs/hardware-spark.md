...
> **S1 stability sweep COMPLETE (2026-09-15, `session/s1-stability-sweep-20260915-0801`); ⚠ its util term is SUPERSEDED (HD-374 2026-09-16 removed `gpu_memory_utilization` — this build ignores it once `kv_cache_memory_bytes` is set) and the live pool has since moved 8.2 → 16 GiB (certified 2026-09-18, §Unified-memory budget): read the current value from `group_vars/spark.yml`, never from this paragraph.** What the sweep certified stands: max ctx **262,144** = model native max_position_embeddings (300k rejected at startup validation — would need `VLLM_ALLOW_LONG_MAX_MODEL_LEN` + YaRN, needle-gated); 250k@95 % fill 0 preemptions; acc-gate 8/10 serial + 8/10 concurrent-3; concurrency accuracy holds. **⚠️ Still pending before full production trust:** needle-test at 262k (long-context correctness, QSA/XQA trap ≥120k), tool-call EMPTY fix (no tools registered), true 3×87k-token fill stress, NVFP4/SGLang lane (S2/S3, the fast-on-GB10 config). **Verdict: production-ready for casual/chat inference now; agentic/max-context work gated on the pending tests.**
>
