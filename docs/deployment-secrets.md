...
| `oldsrv-cockpit_login` | `password` | **HD-361** — the PAM password behind the `maint` break-glass identity on **oldsrv**, which is the only thing standing between `cockpit-oldsrv.kogler.si` and an unusable console. Catalog-generated (the account does not exist yet, so there is no live value to match — the opposite of `cockpit-pi-web_api` above). The consumer renders the hash **from the vault** and grants `sudo` group membership and NOTHING else: no `NOPASSWD` sudoers entry (break-glass keeps the interactive sudo password), no SSH key, excluded from `AllowUsers` — the account is a local-console/Cockpit PAM identity with no network key surface. ⚠ Measured 2026-09-23: `getent passwd maint` on oldsrv → **ABSENT**, so the route is deployed, `cockpit.socket` is active on 9090, and there is no working login. HD-361's own instruction to "document both items in this file in the same change" had never been done — these two rows are that overdue change. |
| `nas-cockpit_login` | `password` | **HD-361** — the same `maint` break-glass PAM identity on **nas** (`cockpit-nas.kogler.si`), same rules, same absence today. |
