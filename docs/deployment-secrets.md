...
| `authentik-provision_api` | `api` | authentik — API token (**write-scoped**, AUTHENTIK-ISSUED — issued in the Authentik UI; interim: ak-shell mint, see services-authentik live-deploy findings) used as the Bearer credential by the OIDC **secret-egress glue**. ⚠ NOT a 1Password service-account token — the 2026-08-22 first deploy conflated the two (journal layers 3–5). Least-privilege scope-down (issuer/app/flow/outpost only) = post-green HD-211 batch. |
