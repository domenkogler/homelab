...
| `Hertzner-SB-Data` | — (connection ref) | Hetzner Storage Box **live** (BX11 1 TB) — *no `<service>_<type>` lookup*; connection reference for CIFS/SMB + WebDAV (`u653411`, server `653411`, SSH/SFTP port 23), **SMB password** stored here. Recorded under `subscriptions.yml` (`secret: Hertzner-SB-Data`), not consumed by an Ansible `onepassword` lookup — see `subscription.md` "Hetzner Storage Box — live" |
