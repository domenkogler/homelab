...
| `op-write_api` | `api` | 1Password **service-account token (read+write)** — renamed 2026-09-19 from `vps-op-write_api`, which is DELETED — for the VPS host-side `op` CLI — deployed by the docker_services pre-pass to `/etc/op/provision-token` (0600); the glue uses it to seed the OIDC client-cred items. Different system/secret from `authentik-provision_api`. |
