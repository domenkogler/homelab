...
| ⚠ `cockpit-pi-web_api` | **does not exist yet** | The `PI_WEB_TOKEN` guarding the oldsrv coding cockpit (HD-409) lives **only** in `/home/domen/.config/pi-web/env` on that box (0600, owner `domen`). It could not be minted into the vault in the same session because the write-scoped SA above is deleted — which is exactly the reason this row exists: a credential with no vault item is invisible to rotation and to the inventory, the same class as `oldsrv-rsync` and the second live `op_api`. ⏳ When the write SA is re-minted: `op item create --category="API Credential" --title cockpit-pi-web_api`, put the SAME value in (the cockpit would otherwise need its env rewritten), verify by hash, and turn this row into a normal catalog entry with a rotation note. |
