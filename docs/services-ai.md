...
| **27** | **⏳ PROPOSED, ratification pending (HD-391) — pinned-AI engines move to the ggml/Vulkan backend family: STT `ghcr.io/ggml-org/whisper.cpp:main-vulkan` (digest-pinned, `large-v3-turbo`, GPU-first with native CPU fallback) and rerank `ghcr.io/ggml-org/llama.cpp:server-vulkan` + `gpustack/bge-reranker-v2-m3-GGUF` Q8_0 (routed as LiteLLM `jina_ai/`), both on the RX 7600.** Re-decides **#25(c)** (GGML_HIP → Vulkan: the HIP recipe has **no published artifact**, so the only alternative is a self-build with no Renovate trail — §9c corrected 2026-09-18) and **#25(b)** (CPU → dGPU: measured **0.50 s vs 5.3 s** for a top-20 rerank at **425 MiB** VRAM instead of the assumed 1.5 GB, and CPU rerank is 10–20× slower than #25(b)'s 10–30 ms/pair premise). **Keeps #24's placement** (RX 7600 = pinned tier) and **leaves embed on Ollama `:rocm` `bge-m3`** (live + E2E verified). Evidence: [services-ai-bench.md](services-ai-bench.md) §2–§7 (device sweep, TEI landmines, iGPU verdict, co-residency ledger: whole pinned tier = **2.9 GiB of 8 GiB**, 0 `amdgpu` hang/reset lines across ~25 min of Vulkan compute). Consequences to accept: two Vulkan clients + one ROCm client share one card (measured contention factor ≈1.5–2× under simultaneous load, flat when sustained); `main-vulkan` + `server-vulkan` are **mutable aliases** ⇒ §7 MUST-pin by digest; the rerank leg gains a GGUF model-fetch task and loses the `huggingface/`-provider wiring. | 2026-09-18 |
...
⇒ **proposed decision #27** (§9 row 27) moves rerank back to the dGPU on Vulkan; owner ratification pending.
