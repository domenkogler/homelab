...
|---------|------|---------|-------|
| **LiteLLM** | LLM gateway / router | `services-internal` **+ `llm-backend`** | OpenAI-compatible spine (v2, Postgres-backed, HD-247). Only component holding upstream keys. Joins `llm-backend` to reach Ollama; serves embed/rerank to Qdrant/rag-mcp. **HD-370:** admin reached at `litellm.kogler.si` **tailnet-only** (+`.ts`; tailnet edge → `http://litellm:4000`, docker DNS) — never public. Its spark generation entry uses `https://llm.kogler.si` (tailnet edge → WG → spark :443) + `spark-llm_api`. |
