...
- **HD-333 spec (WG/tailnet reach for the internal edge — this doc is the SSOT):** **IAC-AUTHORED (2026-09-07, not yet live — deploy-gated on VPS `docker_services` + `vps-hardening` converge).** the internal all-app edge (`traefik-tailnet` growth, HD-331/332) is reached two ways:
