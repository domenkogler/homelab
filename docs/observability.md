...
| **MikroTik SNMP community** — Alloy collector assumes v2c `public` (monitoring role `snmp.yml.j2`) | before network deploy (HD-03) | ✅ **Decided (HD-53, Option A):** dedicated read-only community `network-snmp_api` (in 1Password) + **Mgmt-VLAN-only** ACL on the router INPUT chain (161/udp reachable only from VLAN 99). Device-side `/snmp` enable + RO community = HD-03 deploy step (RouterOS commands in `snmp.yml.j2` header) |
