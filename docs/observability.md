...
| **UPS** | `homelab-ups` | (pre-existing) NUT battery/load/voltages + UPS status panel (nut_ups_status bitmask) | `nut_*` |
...
> - ~~`traefik_requests_total`~~ — **✅ CLOSED 2026-09-04 (IaC):** the Traefik compose now enables a Prometheus metrics endpoint on the main edge (`--metrics.prometheus=true` + `--entryPoints.metrics.address=:8082`, overlay-only — never on a host port). The `traefik` scrape job target (`traefik:8082`) now returns data after the next `docker_services` converge + traefik restart; ⏳ live-verify the series populates (`traefik_requests_total`, `up{job="traefik"}`) once deployed.
