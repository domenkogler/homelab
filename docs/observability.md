...
- ⚠️ **spark host-memory alert — AUTH (rule names: `spark-host-mem-oom-warning` / `spark-host-mem-oom-critical`) — 2026-09-16, deploy-gated; tracked as **HD-375** (todo §2.12).** Three global-OOM incidents on spark in 24 h (2026-09-15 ×2, 2026-09-16 ×1) killed the host user session + the vLLM engine. The container memory cage can't protect the host (GPU pages are not cgroup-charged), and Docker's `OOMKilled` is a false negative — the only early warning is the **host `MemAvailable`** (node_exporter `node_memory_MemAvailable_bytes{job="alloy",instance=~"spark.*"}`) crossing the ~21.9 GiB reserve. **Author**: add the two rules to `roles/monitoring/vars/main.yml` (Warning `MemAvailable < 24 GiB` sustained, Critical `< 18 GiB` or the `restartCount` spike; `noDataState: OK`, `execErrState: Error` per the HD-347 convention). Data now flows (spark Alloy `unix` exporter); Deploy-gated on the next monitoring converge. · [hardware-spark.md](hardware-spark.md) §Unified-memory budget · [spark-incidents.md](spark-incidents.md)
...
| **★ LLM** *(HD-377, ⏳ deploy-gated)* | `homelab-llm` | **the unified inference dashboard — the merge of the three rows above into one board** (owner: "instead of many dashboards, I want 1"): readiness → spark node (CPU/RAM/GPU) → scheduling → latency → throughput → cache → reliability → request shape → per-engine detail → forward-look. Generated by [`scripts/build-llm-dashboard.py`](../scripts/build-llm-dashboard.py); see **§LLM Dashboard** below. The three HD-368 dashboards stay provisioned until the owner signs off. | 43 of 47 query metric names **live in VM today** (the 3 absent = intentional DCGM panels) · `vllm:*` + `node_*{job="alloy",instance="spark.kogler.si"}` + `process_*{job=~"vllm|sglang"}` · every query joined `by (job, instance)` + scoped to `$instance` |
...
**GPU on a GB10 — what is genuinely unavailable (verified live 2026-09-16).**
```
...
```
The GB10 has **no independent VRAM counter** — every CUDA allocation lives in the 121.6
GiB host pool. So the honest GPU picture is: unified-pool RAM (where the allocation
actually is), engine RSS (the process's share of it, vs the HD-374 105 GiB cage) and the
engine's own per-GPU work estimates (`vllm:estimated_flops_per_gpu_total`,
`estimated_read_bytes_per_gpu_total` — live today, plotted in "GPU work — engine
reported"). `DCGM_FI_PROF_SM_ACTIVE` / `PIPE_TENSOR_ACTIVE` / `DRAM_ACTIVE` panels are
**authored empty on purpose**: SM/tensor-pipe activity and power draw exist on no other
exporter, and faking them from host CPU% would print a different quantity under a GPU
label. Wiring DCGM = run `nvidia/dcgm-exporter` (ships arm64) loopback-published on
`:9400` + a `prometheus.scrape "dcgm"` block in `alloy.river.j2` under
`{% if inventory_hostname == 'spark.kogler.si' %}` — then those panels fill with **no
dashboard change**. nvidia-smi *can* still give temp + power; those ride the DCGM row
rather than a bespoke textfile collector, to avoid adding an unmanaged mover on the box
that OOMs (see `spark-incidents.md`).
