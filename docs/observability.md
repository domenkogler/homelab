...
| **LLM Inference (vLLM/SGLang)** | `homelab-llm-inference` | unified vLLM+SGLang inference view (grafana.com gnet 25502): request volume, token throughput, latency quantiles, queue/KV-cache, engine/TP-rank | `vllm:*` / `sglang:*` `{job="vllm"}` (spark) |
| **vLLM Monitoring V2** | `homelab-vllm-monitoring-v2` | vLLM server monitoring (gnet 24756): running/waiting/swapped, KV-cache %, token rates, latency p50/p95/p99, prefix-cache, preemption | `vllm:num_requests_{running,waiting,swapped}` · `vllm:kv_cache_usage_perc` · `vllm:generation_tokens_total` · `vllm:time_to_first_token_seconds_bucket` ... |
| **vLLM Dashboard** | `homelab-vllm-dashboard` | official vLLM production-stack dashboard (gnet 25043): QPS, KV-cache %, latency e2e/TPOT, prefix-cache hits, CPU/mem | `vllm:current_qps` · `vllm:kv_cache_usage_perc` · `vllm:e2e_request_latency_seconds_{sum,count}` · `vllm:gpu_prefix_cache_hits_total` |
