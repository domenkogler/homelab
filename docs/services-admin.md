...
- **Metabase SMTP (HD-241, env-driven):** `MB_EMAIL_SMTP_*` in the compose template consume the shared smtp2go SSOT (`smtp2go_host`/`smtp2go_port`) + the shared `smtp_login` item; From = `notify@kogler.si` (smtp2go account identity). Env-set settings override AND lock the Admin-UI fields → mail config changes via converge only. **✅ SMTP env LIVE-APPLIED 2026-09-08** (container env verified on the VPS). 🔴 **Port fix 2026-09-08:** smtp2go `587` is BLOCKED from the VPS egress (owner's *Send test email* 500 / ~134s; only 2525/8025 open from VPS) → SSOT `smtp2go_port` changed to **2525**; ⏳ VPS surgical converge + owner *Send test email* retest → invite flow.
