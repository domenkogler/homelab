...
- **Metabase SMTP (HD-241, env-driven):** `MB_EMAIL_SMTP_*` in the compose template consume the shared smtp2go SSOT (`smtp2go_host`/`smtp2go_port`) + the shared `smtp_login` item; From = `notify@kogler.si` (smtp2go account identity). Env-set settings override AND lock the Admin-UI fields → mail config changes via converge only. **✅ SMTP env LIVE-APPLIED 2026-09-08** (container env verified on the VPS). ⏳ Owner browser verify: Admin → Email → *Send test email* → invite flow works.
