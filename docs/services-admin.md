...
- **Admin Dashboards decision:** Traefik Dashboard — tailnet-only `traefik.kogler.si` ([`services-traefik.md`](services-traefik.md)); CrowdSec Dashboard — tailnet-only `sec.kogler.si` (Metabase). Both are reached over the **headscale tailnet**, no public record (HD-135b follow-up, 2026-08-28). **Portainer / Dockge — excluded** (single Ansible-templated compose model).
