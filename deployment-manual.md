...
Role order is **load-bearing** (HD-185/204 render-first decision): `common` → `ai_diag` → `network` (static dual-home via **NetworkManager keyfile** — Pi-uses-NM, [network-rejected.md](docs/network-rejected.md) 2026-09-01; RPi OS ships NetworkManager, no systemd-networkd. The role renders `pi-eth0.nmconnection`: Home + Mgmt static per SSOT ([network-addresses-generated.md](docs/network-addresses-generated.md) `pi` rows), default via Home (single gateway — no `never-default`), DNS via `bootstrap_dns_servers`, DHCP off; session-safe — both addresses added while the connection stays up) → `nut` (client, `shutdown_delay_seconds=0`) → `docker` → **`home_assistant` → `docker_services`** → `monitoring` (Alloy only). Running `home_assistant` BEFORE `docker_services` renders `configuration.yaml` / `keepalived.conf` (+ `secrets.yaml` once its renderer lands, HD-185) as **regular files** before first `compose up` — the old KOPS-063 order made Docker auto-create bind-mount dirs and HA silently ran default config. Do not reorder.

> ⚠ **Session-safety (live lesson 2026-09-01):** do **NOT** switch the Pi's NM connection with `nmcli connection up <new-profile>` — activating a new manual profile **deactivates the DHCP connection and drops the interface**, which severed the remote SSH session and took the Pi offline (only the router could ping it on the Home leg). The **session-safe additive pattern** is to keep the DHCP connection and **add the Mgmt address to it** (`nmcli connection modify "Wired connection 1" +ipv4.addresses <mgmt_ip>/24` — per the SSOT); a full static profile can be applied at the console/boot, never over the wire. Rework `roles/network` to the additive form before any live Pi run.
