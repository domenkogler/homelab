...
| 4 | **HD-396** | One question, answered from the repo + the Forgejo side: **does the CI runner still exist, and does it still hold the pre-2026-09-19 `op_api` secret?** Then renew it **or** delete the stale Phase 0/5 prerequisite lines in [deployment-tasks.md](deployment-tasks.md) | The leak itself is closed — the record is [deployment-ai-stack-secrets.md](docs/deployment-ai-stack-secrets.md) §4a, do not re-derive it. No IaC references `op_api` any more, so only the Forgejo side can answer · if the answer needs the Forgejo UI and the owner is absent, **park** it (O4) with the exact question |
