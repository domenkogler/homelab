...
| # | HD | Action | Gate / note |
|---|----|--------|-------------|
| 1 | **HD-417** | ✅ **shipped 2026-09-25 by the parent while the brief was idle** — `scripts/check_md_tables.py`, wired into `validate-all.sh`, with a `--self-test` fixture and the one-time sweep done (11 rows, green on day one) | Wider-only, not equality: 14 narrower rows are legal GFM, so an equality assert printed 41 findings in 12 files and would have been muted inside a week. Escape convention `\|` in cells is in the script docstring — GFM cuts a cell on a pipe **inside a code span**, which is what bit `scripts/README.md` |
