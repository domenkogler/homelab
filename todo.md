...
**Status: 44 open · 6 done** · **Total: 50**
...
| HD-11 | 2 | AI | open | **Create Pi first-boot config** — Pi uses raspi.debian.net images (not preseed). Write `first-boot-config.sh` to configure the SD card boot partition (SSH enable + cloud-init user-data with ansible-admin + ai-debug keys). Done but needs reopening after preseed approach was corrected. · [deployment-preseed.md](docs/deployment-preseed.md) |
