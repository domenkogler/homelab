...
| HD-198 | 2 | AI + gate | 3 | **Public-record SSOT migration (B2)** — `roles/cloudflare_dns/vars/main.yml` manages only `vps` + SMTP2Go CNAMEs; the service records (root/home, sso, foto, file, office, ai, git, ha, vpn, matrix, chat per services.md) exist only as comments → live zone and file are dual SSOTs. Complete the list (gate: records go LIVE on apply), then render the human mirror. Closes audit B2/W7. · [services.md](docs/services.md), [network-dns.md](docs/network-dns.md) |
...
| HD-189 | 2 | AI | 1 | **Gate hardening bundle** — (a) `validate-secrets.py`: scan `templates/**/*.j2` + `roles/*/vars|files` (literal credential in a compose template is currently caught by nothing); (b) `validate-docker-services.py`: fail-open group_vars loader → abort, dedupe `NETWORK_MAP` duplicate keys or delete the dead map, read `_extra_templates` from role defaults instead of the duplicated copy, fix `BASE_CTX` mock drift (`op_vault`, GPU gids) or load from SSOT; (c) new **vault-name lint** (`Homelab` not followed by `-ansible`) + fix the known offenders it finds. · prompt-hd189.md. Closes audit F1.1/F1.2/F1.3/F1.5/F1.8/F7/F11(partial). · [scripts/README.md](scripts/README.md), [scripts.md](scripts.md) |
| HD-192 | 3 | AI | 1 | **Version-pin completion (S2/D1)** — 21 templates on bare `:latest` + mutable aliases (`ollama:rocm`, HA `stable`, immich `release`); add `*_version` vars to `versions.yml` (registry-verify each once), remove `\| default('latest')` fallbacks (CONVENTIONS §7: no default()), invert the validator allowlist to a zero-entry denylist. · prompt-hd192.md. Closes audit S2/D1/F1.6. · [deployment-compose.md](docs/deployment-compose.md), [security.md](docs/security.md) §2 |
...
| ID | D | Exec | P | Item |
|----|---|------|---|------|
| HD-181 | 3 | AI + gate | 1 | **Single cert issuer (HD-178): `acme_issuer` flag + doc alignment** — · prompt-hd181.md. — parameterize the traefik template so ACME + certs-dumper run on the **VPS only** (oldsrv/Pi consume synced certs); align the four stale texts (`all.yml` letsencrypt comment, smart-home-failover.md, IaC/README.md, deployment-tasks Phase 3); add the S1 verify line to the services-vps checklist. **Docs aligned ✅ 2026-08-21** (all.yml, smart-home-failover, IaC/README, deployment-tasks Phase 3, services-traefik, services-vps, traefik template header); remaining = IaC mechanism + verify line. Closes audit J3/C1/W11. · [services-traefik.md](docs/services-traefik.md), [services-vps.md](docs/services-vps.md) |
...
| HD-185 | 3 | AI + gate | 1 | **Fix Pi first-deploy ordering (docker_services vs home_assistant)** — KOPS-063 order auto-creates `config/`, `secrets.yaml`, `keepalived.conf` as empty **directories** at first `compose up`; the HA role's `copy → keepalived.conf` then fails and HA runs default config. Pick + implement a fix (render-first reorder vs pre-created files vs `create_host_path: false`) after re-reading the KOPS-063 rationale. · prompt-hd185.md. Closes audit J2/D2. · [smart-home-failover.md](docs/smart-home-failover.md), [deployment-ansible.md](docs/deployment-ansible.md) |
...
| HD-194 | 1 | AI + gate | 3 | **`sso` route edge middleware (S17a)** — the public login page carries zero edge middleware (law exception, undocumented). Add `crowdsec-only@file` (verify Authentik outpost/OIDC callbacks aren't bounced) or document the exception + compensating control (fail2ban http-auth jail) in security.md §1. · Closes audit S17(a). · [security.md](docs/security.md) §1, [services-traefik.md](docs/services-traefik.md) |
| HD-202 | 2 | AI + gate | 3 | **Container-hardening policy (S22/S21)** — decide + implement: roll `cap_drop: ALL`/`read_only` into templates per deployment-compose conventions (GPU/VPN/gluetun exempt) OR enforce via validator with a documented allowlist; document the one intentional `default('', true)` (signal captcha) or replace it. · Closes audit S21/S22. · [deployment-compose.md](docs/deployment-compose.md) §Container Security |
...
| HD-191 | 2 | AI | 2 | **oldsrv Kopia agent (S18/J5)** — backup.md says Kopia runs on VPS + oldsrv, but no agent exists in `group_vars/home_servers.yml` → oldsrv-local state (dumps, service state, thumbs, `/opt/*` configs) has no off-site path. Add the agent (kopia-server API over WG, `kopia-server-internal_api`) + sources/policy per backup.md; align backup.md if the design changes. · prompt-hd191.md. Closes audit S18/J5/W12. · [backup.md](docs/backup.md) |
