...
| HD-132 | 3 | AI | 2 | **Implement Authentik-as-LDAP for Samba self-service password (D7/HD-131)** — switch Samba to authenticate against **Authentik as an LDAP provider** (`passdb backend = ldapsam`, LDAPS/TLS); users change their own password in the Authentik self-service portal (no admin). Requires: Authentik LDAP provider + bind DN/base DN secrets (`authentik-ldap_bind*` → 1Password), Samba `passdb backend=ldapsam`, TLS handshake. Keep D5 glue for dataset+share+OpenCloud; password ownership moves from D5 local-`smbpasswd` to Authentik-LDAP. Passkey-only users set one-time workspace LDAP password in the portal. **Assigned to a later session; not yet implemented.** · [deployment-compose.md](docs/deployment-compose.md), [storage-zfs.md](docs/storage-zfs.md) |
