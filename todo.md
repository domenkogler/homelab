...
| HD-94 | 2 | AI | 2 | **Centralize shared-data owner as `storage_uid`/`storage_gid` (KOPS-060)** — per **HD-131 (D1/D2) storage-access decision**, centralize for the **filesystem/SMB-backed** services (shared-media *arr stack, OpenCloud local user, Samba-shared media), NOT the S3-backed Immich originals. Replace hardcoded `PUID/PGID: 1000:1000` + `user: "1000:1000"` literals in those compose templates with the group_vars `storage_uid`/`storage_gid`; define the neutral shared owner once (e.g. `1005`) in the storage role group_vars. Also stand up the Samba per-user + shared-media shares + the Authentik→NAS provisioning glue (HD-131 D4/D5). Verify NFS/SMB ownership + container perms after. · [deployment-compose.md](docs/deployment-compose.md), [hardware-nas.md](docs/hardware-nas.md) |
