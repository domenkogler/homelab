...
| HD-187 | 1 | AI | 2 | **Pi-hole conditional-forwarding points at the HA VIP** — ✅ **Done (IaC).** `CONDITIONAL_FORWARDING_IP: "{{ ha_vip }}"` (the VIP serves HA/edge, not DNS) → now `dns_primary_ip` (Technitium primary per host_vars SSOT); validator render context gained the matching instance-specific mock; render check green. Closes audit J4/D4. · [network-dns.md](docs/network-dns.md) |
...
| HD-188 | 1 | AI | 2 | **Cockpit routes: derive IPs + add edge middleware** — ✅ **Done.** `roles/cockpit/templates/cockpit-routes.yml.j2`: backends derived from the address SSOT (`network_static_hosts` name+vlan lookup — renders byte-equivalent to the old literals), both routers now carry `crowdsec-only@file` (§1 law); TLS already consumer default-store (HD-181). services-traefik.md cockpit section updated (derivation + middleware noted; render-verified against real group_vars). Closes audit J9/D6/S17(b). · [services-traefik.md](docs/services-traefik.md) |
...
| HD-196 | 2 | AI | 3 | **Stale-docs sweep (VPS era)** — ✅ **Done.** Every listed stale verified against current content first; already-fixed by HD-181…202 merges skipped (date typos swept by HD-195; security.md §2 "currently latest"/"42 templates" fixed by HD-192; VPN/UPS dispatch rows already present). Fixed now: backup.md DB/service-state locations → VPS NVMe split + TSDB-exclusion note → VPS; observability.md Loki "oldsrv NVMe" → VPS + the two decided TODO rows (Alloy instance label HD-116, SNMP community HD-53) moved to a Decided note; services-matrix.md placement/storage-path/WAN-443/RAM → VPS (+ authoring-spec group_vars pointer vps.yml, backup row de-Postgresed); hardware-nas.md TODO box deleted (storage role implemented); storage.md "Proposed IaC stub" section incl. 3879 typo deleted + legacy Immich block ≤5 lines; security.md §9 duplicated fragment removed; index.md storage-triage dispatch row added + ★ re-starred by actual role (generated views + finance de-starred, storage starred, legend reworded); network-dns `bck` ghost removed (also services-admin Kopia row), public-record lists aligned to services.md mirror, Pi-hole config merged into services-dns.md (pointer left); legacy blocks ≤5 lines in deployment-compose.md (Immich) + hardware-oldsrv.md (Proxmox); interfaces.md pipeline step 5 marked control-plane-manual (D11). · Closes audit G1–G4, §1 docs-changes. · [docs-changes.md](docs-changes.md) |
