...
| HD-162 | 2 | AI | 3 | **docker_services: move Authentik glue to an explicit pre-pass** — blurry deploy/edge; make pre-pass task + render/validate step inside loop. · [deployment-ansible.md](docs/deployment-ansible.md) |
| HD-163 | 2 | AI | 3 | **Consolidate render scripts behind one entry (`render_all.py`)** — thin wrappers over `render-docs.yml` path. · [scripts/](scripts/) |
