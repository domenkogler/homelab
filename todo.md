...
| HD-46 | 4 | AI | open | **Implement Matrix IaC (native-only)** — `docker_services` templates + `group_vars/home_servers.yml`: Tuwunel homeserver (`matrix.`) + Element Web (`chat.`); SSO → Authentik OIDC; secrets → 1Password `Homelab`. ⚠ **No bridges in Phase 1** (deferred — HD-48). · [services-matrix.md](docs/services-matrix.md) |
