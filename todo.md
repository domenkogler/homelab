...
| HD-189 | 2 | AI | 1 | **Gate hardening bundle** — ✅ **Decided (HD-204, 2026-08-21): delete the dead `NETWORK_MAP`.** (a) `validate-secrets.py`: scan `templates/**/*.j2` + `roles/*/vars|files` (literal credential in a compose template is currently caught by nothing); (b) `validate-docker-services.py`: fail-open group_vars loader → abort, delete the dead `NETWORK_MAP` (decided) + dedupe any remaining duplicate keys, read `_extra_templates` from role defaults instead of the duplicated copy, fix `BASE_CTX` mock drift (`op_vault`, GPU gids) or load from SSOT; (c) new **vault-name lint** (`Homelab` not followed by `-ansible`) + fix the known offenders it finds. · prompt-hd189.md. Closes audit F1.1/F1.2/F1.3/F1.5/F1.8/F7/F11(partial). · [scripts/README.md](scripts/README.md), [scripts.md](scripts.md) |
| HD-192 | 3 | AI | 1 | **Version-pin completion (S2/D1)** — ✅ **Decided (HD-204, 2026-08-21): `tuwunel`/`litellm` remain documented MUST-pin exceptions (not pinned this pass).** 21 templates on bare `:latest` + mutable aliases (`ollama:rocm`, HA `stable`, immich `release`); add `*_version` vars to `versions.yml` (registry-verify each once), remove `\| default('latest')` fallbacks (CONVENTIONS §7: no default()), invert the validator allowlist to a zero-entry denylist. · prompt-hd192.md. Closes audit S2/D1/F1.6. · [deployment-compose.md](docs/deployment-compose.md), [security.md](docs/security.md) §2 |
