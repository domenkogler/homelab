...
| HD-40A | 3 | AI + Human | 1 | *(Phase 1)* **Provision VPS + establish public Traefik edge** — netcup **RS 2000 G12** purchased (263,52 €/12 mo, 2026-08-18) ≈ formerly “Contabo”. ✅ **Provisioned (2026-08-18):** public IP `159.195.111.66` (+ IPv6) + SSH fingerprints recorded (`host_vars/vps.kogler.si.yml`, `deployment-ansible.md`); VPS-specific `post_install.sh` is the **operative** netcup Custom-Script hook (the VPS preseed.cfg `d-i` lines do **not** run on netcup's pre-built image — reference only, `deployment-preseed.md` → VPS Deviations); specs filled (AMD EPYC 9645, 8c/16 GB/512 GB NVMe). **VPS = deployment-track Phase 1** (needed before Phase 2; network redo is Phase 1.5 — see `deployment-tasks.md`). ⏳ **Remaining:** enable `traefik`/`crowdsec`/`authentik` + co-located DBs in `vps.yml` (edge tier, self-contained), Traefik wildcard cert via ACME DNS-01, add `sso` record via `dns.yml`, live-Box CIFS mount; WireGuard S2S home↔VPS (HD-03 WG VPS peer) for backends. · [services-vps.md](docs/services-vps.md), [deployment-tasks.md](../deployment-tasks.md) |
...
| HD-135 | 4 | AI + gate | 2 | **VPS/oldsrv service split — apply to IaC (HD-93 live-edge, nothing deployed yet)** — VPS is now **provisioned** (netcup RS 2000 G12, IP `159.195.111.66`, HD-40A) & both **Hetzner Storage Boxes bought** (HD-31). Reconcile the still-stale “all-on-oldsrv” IaC/docs with the day-one-edge decision. **Placement (locked this task):** · **VPS (public edge + live-data apps + their co-located DBs on NVMe):** traefik, crowdsec, authentik(+pg/redis), open-webui, **litellm** (+pgvector, docling), opencloud(+db), forgejo(+db), immich-app(+pg/valkey; originals→live Hetzner box CIFS; ML→oldsrv), kopia-server, db-backup, openclaw · **oldsrv (GPU + LAN/storage backends):** ollama, immich-ml, jellyfin (server+iGPU transcode — no remote-transcode split exists, `media.` public route via VPS Traefik→oldsrv over WG), sunshine, minio (→retire? Hetzner box supersedes), technitium, pihole, headscale, n8n, media/*arr, pairdrop, stirling-pdf, observability (prom/loki/alloy/grafana/dozzle), ha-standby+keepalived, internal traefik-for-ha. ✅ **Storage+backup transport committed (2026-08-18):** Immich originals+encoded-video → **live Box (CIFS, not S3/MinIO)** incl. Service↔Storage placement table (`storage-zfs.md`, `d805f6d`); **Kopia = SSH/SFTP to backup Box** (port 23, `kopia_sftp_*`, `kopia-s3_api` retired, `fb625d2`). **VPS = deployment Phase 1** (network = Phase 1.5, `deployment-tasks.md`). ⏳ **Remaining (this row):** apply `enabled:` matrices in `home_servers.yml`+`vps.yml` + reconcile stale prose + **Cross-host re-wiring over `wg-vps-services` (10.255.20.0/24):** litellm→ollama (`api_base http://ollama:11434` on `llm-backend` MUST become WG address), immich-app→immich-ml, VPS Traefik→oldsrv backends; co-located DBs move with apps.  ⏳ Deploy-gated: apply after WG S2S + live-box mounts are in place; live-verify a `foto`/`file`/`git`/`ai` round-trip over the tunnel. · [services-vps.md](docs/services-vps.md), [backup.md](docs/backup.md), [services.md](docs/services.md), [storage-zfs.md](docs/storage-zfs.md) — Immich originals+encoded-video→live Box (CIFS, not S3/MinIO); storage-placement table added
...
|----|------------------------------|-----------|
| HD-03 | Network redo — VLAN/firewall/CAPsMAN/WG not deployed to live gear (deploy runnable via Ansible in WSL Debian now — see `deployment-ansible.md` render note); open switch bridge-VLAN membership, CAPsMAN SSID secrets, WG VPS peer | [network-vlans.md](docs/network-vlans.md) |
| HD-135 | VPS/oldsrv split — apply `enabled:` matrices + cross-host WG wiring (litellm→ollama, immich-app→immich-ml) after WG S2S + live-box CIFS mount; live-verify `foto`/`file`/`git`/`ai` round-trip | [services-vps.md](docs/services-vps.md), [storage-zfs.md](docs/storage-zfs.md) |
